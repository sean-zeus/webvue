...
module.exports = {
  ...
  devServer: {
    ...
    port: 5001, // (默认值：8080)
    https: false // (默认值：false),
    // compress: true,
    // disableHostCheck: true,
    // proxy: 'http://localhost:8000/' // 這裡寫你調用接口的基礎路徑，來解決跨域，如果設置了代理，那你本地開發環境的axios的baseUrl要寫為 ''(包含config裏的env.js,url.js和src裏的config\index.js) ，即空字符串
  }
  // chainWebpack: config => {
